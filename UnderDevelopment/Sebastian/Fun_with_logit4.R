...
#Hvad hvis man bootstrapper 10 forskellige?
n_bootstraps <- 10
data_perc <- 0.5 #To get independence

my_preds <- matrix(list(), nrow=nrow(validation_set), ncol=n_bootstraps)

for (i in 1:10) {
  print(i)
  my_rows <- sample(nrow(data), floor(nrow(data)*0.5), replace = T)
  data_subset <- data[my_rows,]
  data_subset <- data
  ...
  my_preds[,i] <- predict(final_model, newdata = validation_set, type = 'response')
}

## Predict
hej <- matrix(unlist(my_preds),nrow = nrow(validation_set),ncol = 10)
predictions <- rowMeans(matrix(unlist(my_preds),nrow = nrow(validation_set),ncol = 10))
correct <- fold2$label
