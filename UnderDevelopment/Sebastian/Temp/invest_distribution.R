...
### DATA
data <- read.table("/Users/SebastianGPedersen/Google Drive/LargeScale/Processed_Data/Sebastian/fold3_NA_features.csv",
                   sep = ",",
                   header = TRUE)
...
data2 <- data
data2[,3:18] <- sign(data[,3:18]) * abs(data[,3:18])^(1/2)

tmp <- lapply(names(data2), function(x) 
  ggplot(data = data2[, x, drop = FALSE]) + 
    aes_string(x) + xlab(x) + ylab(""))
gd <- geom_density(adjust = 2, fill = gray(0.5))
gh <- function(width) {geom_histogram(binwidth = width)}
...
data2[,19:34] <- sign(data[,19:34]) * abs(data[,19:34])^(1/2)
#data2[,19:36] <- data[,19:36]
...
data2[,35:50] <- log(data[,35:50])
data2[is.na(data2)] <- 0
data2[is.infinite(data2[,37:54])] <- 0
...
data2[,55:66] <- sign(data[,55:66]) * abs(data[,55:66])^(1/4)
